...
| POST | `/v1/treasury-plan-management-plan/initiate` | Initiate |
| GET | `/v1/treasury-plan-management-plan` | Retrieve (list) |
| GET | `/v1/treasury-plan-management-plan/{crId}/retrieve` | Retrieve |
| PUT | `/v1/treasury-plan-management-plan/{crId}/update` | Update |
| PUT | `/v1/treasury-plan-management-plan/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/treasury-plan-management-plan/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-corporate-treasury:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-corporate-treasury ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-corporate-treasury` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
